...
def log_validation(flux_transformer, controlnet, args, accelerator, weight_dtype, step, is_final_validation=False):
...
def parse_args(input_args=None):
    ...
    if (
        ...
        and args.validation_mask is not None
        and args.validation_prompt is not None
        and len(args.validation_image) != 1
        and len(args.validation_mask) != 1
...
def main(args):
    ...
    flux_controlnet = FluxControlNetModel.from_pretrained(args.controlnet_model_name_or_path, torch_dytpe=weight_dtype)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                ...
                FluxControlNetInpaintingPipeline.save_lora_weights(
                    output_dir,
        ...
        def load_model_hook(models, input_dir):
            ...
            lora_state_dict = FluxControlNetInpaintingPipeline.lora_state_dict(input_dir)
            ...
            if args.train_norm_layers:
                ...
                transformer_._transformer_norm_layers = FluxControlNetInpaintingPipeline._load_norm_into_transformer(
                    transformer_norm_state_dict,
    ...
    text_encoding_pipeline = FluxControlNetInpaintingPipeline.from_pretrained(
        args.pretrained_model_name_or_path, transformer=None, vae=None, torch_dtype=weight_dtype, controlnet=flux_controlnet,
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(flux_transformer):
                ...
                pixel_latents_tmp = encode_images(pixel_values, vae.to(accelerator.device), weight_dtype)
                pixel_latents = FluxControlNetInpaintingPipeline._pack_latents( # 1, 2304, 64
                    pixel_latents_tmp,
                ...
                control_latents = torch.concat([control_latents, masks], dim=1) # B, 16+1, 96, 96
                controlnet_cond = FluxControlNetInpaintingPipeline._pack_latents( # 1, 2304, 68
                    control_latents,
                ...
                with torch.no_grad():
                    prompt_embeds, pooled_prompt_embeds, negative_prompt_embeds, negative_pooled_prompt_embeds, text_ids = text_encoding_pipeline.encode_prompt(
                ...
                # print('a', text_ids.shape, latent_image_ids.shape)
                controlnet_block_samples, controlnet_single_block_samples = flux_controlnet(
                    hidden_states=noisy_model_input, # 1, 2304, 64
                    controlnet_cond=controlnet_cond, # 1, 2304, 68
                    ...
                    img_ids=latent_image_ids, # 1, 2304, 3
                    return_dict=False,
                )
                
                # Predict.
                model_pred = flux_transformer( # 1, 2304, 64
                    hidden_states=noisy_model_input, # 1, 2304, 64
                    timestep=timesteps / 1000,
                    guidance=guidance_vec,
                ...
                # 推理时使用！！
                # model_pred = FluxControlNetInpaintingPipeline._unpack_latents(
                #     model_pred, # model_pred:1, 2304, 64
                #     height=pixel_latents_tmp.shape[2] * vae_scale_factor // 2 , # model_pred:1, 2304, 64   pixel_latents_tmp:1, 16, 96, 96    vae_scale_factor: 16
                #     width=pixel_latents_tmp.shape[3] * vae_scale_factor // 2,
                ...
                # flow-matching loss
                target = noise - pixel_latents # 1, 2304, 64
                loss = torch.mean(
